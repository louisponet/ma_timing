...
impl<'a> Timer<'a> {
    pub fn new<S: Display>(name: S) -> Self {
        ...
        let timing_queue =
            ma_queues::Queue::shared(format!("{QUEUE_DIR}/timing-{name}"), QUEUE_SIZE, ma_queues::QueueType::SPMC).expect("couldn't open timing queue");
        let latency_queue =
            ma_queues::Queue::shared(format!("{QUEUE_DIR}/latency-{name}"), QUEUE_SIZE, ma_queues::QueueType::SPMC).expect("couldn't open latency queue");
...
impl<'a> Timer<'a> {
    ...
    pub fn stop(&mut self) {
        ...
        self.timing_producer.produce(&messages::TimingMeasurement::TwoStamps(self.curmsg));
    }
    #[inline(always)]
    pub fn latency(&mut self, ingestion_t: Instant) {
        let curt = Instant::now();
        self.latency_producer.produce(&messages::LatencyMeasurement::TwoStamps(messages::LatencyMessage {
            ingestion_t,
            arrival_t: curt,
        }));
    }
    ...
    pub fn latency_nanos(&mut self, nanos: Nanos) {
        self.latency_producer.produce(&messages::LatencyMeasurement::Interval(nanos));
    }
}
